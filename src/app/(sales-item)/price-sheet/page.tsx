...
const Page = () => {
    ...
    useEffect(() => {
        fetch(`${apiBaseUrl}/sales/product-rates`)
                ...
                <div className="overflow-x-auto">
                    ...
                    <div ref={contentToPrint} className="flex-1 p-5">
                        <div className="flex flex-col items-center justify-center w-full pb-5">
                            <h4 className="font-semibold text-lg">PRODUCT PRICE SHEET</h4>
                            <h4><CurrentDate /></h4>
                        </div>
                            ...
                            <tbody>
                                {filteredProducts?.map((product, index) => (
                                    <tr key={index}>
                                        ...
                                        <td>{Number((product?.unitRate ?? 0).toFixed(2)).toLocaleString('en-IN')}</td>
                                        <td>{Number((product?.saleRate ?? 0).toFixed(2)).toLocaleString('en-IN')}</td>
                                        <td>{Number((product?.qty ?? 0).toFixed(2)).toLocaleString('en-IN')}</td>

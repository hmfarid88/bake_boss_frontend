    ...
    const handleUserUpdate = async (e: FormEvent<HTMLFormElement>) => {
        ...
        try {
            const response = await fetch(
                `${apiBaseUrl}/auth/updatePassword?username=${encodeURIComponent(username)}&currentPassword=${encodeURIComponent(currentPassword)}&newPassword=${encodeURIComponent(newPassword)}`,
                { method: 'PUT' }
            );

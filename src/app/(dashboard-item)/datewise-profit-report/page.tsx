...
const Page = () => {
  ...
  return (
    <div className="container-2xl">
      <div className="flex flex-col w-full gap-5 min-h-[calc(100vh-228px)] p-4 items-center justify-center">
      <div className="flex">
        <DateToDate routePath="/datewise-profit-report" />
        </div>
          ...
          <h4>{startDate} TO {endDate}</h4>
            ...
            <table className="table">
              ...
              <tr className="font-semibold text-lg">
                  <td colSpan={3}></td>
                  <td>TOTAL</td>
                  <td>{Number((totalCostValue).toFixed(2)).toLocaleString('en-IN')}</td>
                  <td>{Number((totalSaleValue).toFixed(2)).toLocaleString('en-IN')}</td>
                  <td></td>
                  <td>{Number(totalQty.toFixed(2)).toLocaleString('en-IN')}</td>
                  <td>{Number(((totalSaleValue)-(totalCostValue)).toFixed(2)).toLocaleString('en-IN')}</td>
                </tr>
              </tfoot>
            </table>
          </div>

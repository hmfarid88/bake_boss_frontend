...
const Page = () => {
    ...
    useEffect(() => {
        fetch(`${apiBaseUrl}/supplierBalance/supplier/details?username=${username}&supplierName=${supplierName}`)
        ...
        <div className="container-2xl">
            <div className="flex w-full min-h-[calc(100vh-228px)] p-4 items-center justify-center">
                <div className="overflow-x-auto">
                    <div className="flex justify-center uppercase p-3 font-bold">  <h4>SUPPLIER NAME: {supplierName}</h4></div>
                            ...
                            <tbody>
                                {filteredProducts?.map((product, index) => (
                                    <tr key={index}>
                                        ...
                                        <td>{product.materialsQty}</td>
                                        <td>{product.materialsRate.toFixed(2)}</td>
                                        <td>{Number((product.materialsQty * product.materialsRate).toFixed(2)).toLocaleString('en-IN')}</td>
